  ...
  <div class="min-h-screen flex items-center justify-center p-6 bg-black-main relative">
    ...
    <div class="w-full max-w-md relative z-10">
      ...
      <div class="text-center mb-8">
        ...
        <p class="text-gray-400 text-sm">Get started with Private Connect</p>
      </div>
      ...
      <div v-if="success" class="bg-gray-500/10 border border-gray-500/10 rounded-lg p-8 animate-fade-in">
        <div class="text-center">
          ...
          <p class="text-gray-400 mb-4">
            We sent a verification link to <span class="text-white font-medium">{{ email }}</span>.
          </p>
        ...
        <!-- <div class="mt-6 pt-6 border-t border-slate-800 text-center">
          <button @click="reset" class="text-accent-400 hover:underline text-sm">
      ...
      <div v-else class="bg-gray-500/10 border border-gray-500/10 rounded-lg p-8 animate-fade-in">
        <form @submit.prevent="handleRegister">
          <div class="mb-5">
            <label for="register-workspace" class="block text-sm font-medium text-white mb-2">
              Workspace name
            </label>
            <input
          ...
          <div class="mb-6">
            <label for="register-email" class="block text-sm font-medium text-white mb-2">
              Email address
            </label>
            <input
              id="register-email"
          ...
          <button
            type="submit"
            :disabled="loading || !email.trim() || !workspaceName.trim()"
        ...
        <div class="mt-4">
          <p class="text-sm text-gray-400 text-center">
            ...
            <NuxtLink to="/login" class="text-blue-300 hover:underline ml-1">
              Sign in
            </NuxtLink>
          </p>
        </div>
...
const { register, user, fetchCurrentUser } = useAuth();
...
const handleRegister = async () => {
  ...
  try {
    await register(email.value.trim(), workspaceName.value.trim());
    success.value = true;
  } catch (e: any) {

<template>
  <div class="min-h-screen bg-black-main">
    ...
    >
      <nav v-if="mobileMenuOpen" class="md:hidden border-b border-gray-500/10 bg-[#09090b]/95 backdrop-blur-xl">
        <div class="max-w-7xl mx-auto px-6 py-2 flex flex-col gap-1">
          <NuxtLink
            v-for="item in navItems"
            :key="item.path"
            :to="item.path"
            :class="[
              ...
              isActive(item.path) ? 'text-white bg-gray-500/20' : 'text-gray-400 hover:text-white hover:bg-gray-500/10'
            ]"
